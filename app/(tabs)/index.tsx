...
function LangPicker({ value, onChange }: { value: string; onChange: (c: string) => void }) {
  ...
  return (
    <View style={{ zIndex: 100 }}>
      <Pressable onPress={() => setOpen((o) => !o)} style={tStyles.langButton}>
        <Text style={{ fontSize: 18 }}>{current.flag}</Text>
        <Text style={tStyles.langButtonText}>{current.label}</Text>
        <Text style={tStyles.langChevron}>{open ? "▲" : "▼"}</Text>
      </Pressable>
      {open && (
        <View style={tStyles.langDropdown}>
...
const tStyles = StyleSheet.create({
  ...
  langOptionText: { color: "rgba(255,255,255,0.7)", fontSize: 14, fontWeight: "500" },
